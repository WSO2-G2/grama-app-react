...
export default function Status() {
  ...
  const createPDF = () => {
    ...
    doc.text(20, 60, 'Name : ', {maxWidth:'150'})
    doc.text(20, 70, 'NIC : ', {maxWidth:'150'})
    doc.text(20, 80, 'Address : ', {maxWidth:'150'})
    doc.text(20, 100, `Identity Check : ${(identityCheck)?'Verified & Validated':'Unidentified Identity'}`, {maxWidth:'150'})
    doc.text(20, 110, `Police Check : ${(identityCheck)?'Verified & No Crimes found':'Identified with Crimes on the Police records'}`, {maxWidth:'150'})
    doc.text(20, 120, 'Address Check : ', {maxWidth:'150'})
    doc.text(20, 150, 'I cereby certify that the Identity, Police and Address check of the above specified person has been verified.', {maxWidth:'150'})
    doc.text(20, 170, 'Grama Niledari', {maxWidth:'150'})
    doc.setFontSize(10)
    doc.text(20, 180, `Generated on ${(new Date().toJSON().slice(0,10))} `, {maxWidth:'150',align:'left'})
